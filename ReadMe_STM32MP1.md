# Getting Started with STSAFE-A120 Examples on STM32MP1

This guide explains how to build and run the STSAFE-A120 example applications on the **STM32MP1** platform running **OpenSTLinux** (Linux userspace on the Cortex-A7 core).

![STSAFE-A120 Examples on STM32MP1](Documentation/resources/Markdown/00_introduction/Software_package_architecture.png)
...
│   ├── Platform_Linux (Linux platform abstraction)
...
All examples are built using the provided `Linux/Makefile`.
...
```bash
scp Linux/build/01_Echo_loop root@<board-ip>:/home/root/
```
...
i2cdetect -y 5   # Replace 5 with your actual I2C bus number
...
### Linux Platform Layer (`Platform_Linux/`)
...
i2cdetect -y 5      # Scan bus 5 for connected devices
```

### Permission denied accessing I2C

```
i2c_init: failed to open /dev/i2c-5: Permission denied
```
